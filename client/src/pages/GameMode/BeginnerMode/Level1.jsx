...
export default function Level1({ onComplete }) {
  ...
  const handleSubmit = () => {
    // Check if the user has declared a variable and assigned a value in Python
    if (
      userCode.includes("=") &&
      !userCode.includes("console") &&
      userCode.trim() !== ""
    ) {
      setMessage("🎉 Quest Completed! You stored a value in a variable.");
      setLevelCompleted(true); // Unlock Next Level button
    } else {
      setMessage("❌ Try again! Declare a variable and assign a value in Python.");
    }
  };
  ...
  return (
    <div className="bg-gray-700 p-6 rounded-2xl shadow-xl text-center">
      <h2 className="text-3xl font-bold mb-4">🐍 Level 1: Python Variable Quest</h2>
      <p className="text-white/80 mb-4">
        Declare a variable in Python and store a value in it. Example: <code>x = 10</code>
      </p>

      <textarea
        value={userCode}
        onChange={(e) => setUserCode(e.target.value)}
        placeholder="# Example: myVar = 5"

...
import { FaCommentDots, FaChevronRight, FaLightbulb, FaGamepad, FaCheckCircle, FaStar, FaArrowLeft, FaGem, FaFire } from "react-icons/fa";
...
const QUIZ = [
  {
    ...
    options: ["Prints on paper 🖨️", "Shows text on screen 💻", "Deletes your code 🗑️", "Opens a window 🪟"],
    answer: 1,
  },
  {
...
export default function Level1() {
  ...
  const [step, setStep] = useState(0); // 0=intro, 1=learn, 2=tryit, 3=quiz, 4=complete
  ...
  const handleQuizAnswer = (idx) => {
    ...
    setQuizSelected(idx);
    const correct = QUIZ[quizIdx].answer === idx;
    if (correct) {
      setQuizScore((s) => s + 1);
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            <motion.div key="intro" {...fadeUp} className="text-center">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200, delay: 0.2 }}
                className="inline-block bg-gradient-to-r from-green-400 to-green-500 text-white px-8 py-2 rounded-full font-black text-sm mb-6 shadow-lg shadow-green-200">
                LEVEL 1 — SAY HELLO
              </motion.div>
              ...
              <h1 className="text-4xl font-black text-gray-900 mb-4" style={{ fontFamily: FONT }}>
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {[["🐍", "Python is a real programming language!"], ["💬", "print() is used by millions of coders"], ["🚀", "Used in AI, Games & Space Rockets!"]].map(([icon, text], i) => (
          ...
          {step === 1 && (
            <motion.div key="learn" {...fadeUp}>
              ...
              {/* Concept cards */}
              {[
                { title: "What is print()?", color: "bg-green-50 border-green-200", icon: "💡", content: "print() tells Python to show text. Whatever you put inside the parentheses () appears on the screen!" },
                { title: "The Magic Quotes", color: "bg-blue-50 border-blue-200", icon: "✍️", content: 'Text must go inside quotes! You can use "double quotes" or \'single quotes\' — Python accepts both!' },
                { title: "Watch It Work", color: "bg-yellow-50 border-yellow-200", icon: "👁️", content: null, code: true },
              ].map((card, i) => (
                  ...
                  {card.code && (
                    <div className="space-y-3">
                      ...
                      <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm border border-gray-700">
                        <div className="text-gray-400 text-xs mb-2 font-sans font-bold">▶ OUTPUT:</div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
                ...
                <div>
                  ...
                  <p className="text-purple-600 text-sm font-semibold">Python's print() function can print numbers, text, and even emojis! Try print("🎉") sometime!</p>
                </div>
              </motion.div>
          ...
          {step === 2 && (
            <motion.div key="tryit" {...fadeUp}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-black text-gray-900 text-xl" style={{ fontFamily: FONT }}>Try It! 🎮</h2>
              ...
              <AnimatePresence>
                {codeOutput && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0 }}
                    ...
                    <div className="text-gray-400 text-xs font-sans font-bold mb-2">▶ OUTPUT:</div>
              ...
              <div className="bg-amber-50 rounded-2xl p-5 border border-amber-200 mb-6">
                <h3 className="font-black text-amber-800 mb-3 flex items-center gap-2"><FaStar className="text-amber-500" /> Mini Challenges</h3>
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                  ...
                  <div className="flex gap-3 justify-center">
                    <button onClick={() => { setQuizIdx(0); setQuizScore(0); setQuizSelected(null); setQuizDone(false); }}
                      className="px-6 py-3 bg-gray-100 text-gray-600 font-black rounded-2xl hover:bg-gray-200 transition-colors text-sm">Retry Quiz</button>
                    <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setStep(4)}
                      className="px-8 py-3 font-black text-white rounded-2xl bg-gradient-to-r from-green-400 to-green-500 shadow-lg text-sm">
                      Finish Level! 🎉
                    </motion.button>
                  </div>
                </motion.div>
          ...
          {step === 4 && (
            <motion.div key="complete" {...fadeUp} className="text-center">
              <motion.div animate={{ rotate: [0, -10, 10, 0], scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 2 }}
                className="text-8xl mb-6 mx-auto w-fit">🏆</motion.div>
              <h1 className="text-4xl font-black text-gray-900 mb-3" style={{ fontFamily: FONT }}>Level 1 Complete!</h1>
              <p className="text-gray-500 text-lg mb-6">You mastered <strong className="text-green-600">print()</strong>! Your computer can now speak! 🐍</p>
              ...
              <div className="bg-green-50 rounded-2xl p-5 border border-green-200 mb-8 text-left">
                <h3 className="font-black text-green-800 mb-3">🎓 What You Learned:</h3>
                <ul className="space-y-2">
                  {["print() shows text on screen", "Text goes inside quotes (\"like this\")", "You can print words, sentences and emojis!"].map((item, i) => (
              ...
              <button onClick={() => navigate("/dashboard")} className="mt-4 text-gray-400 font-bold text-sm hover:text-green-500 transition-colors">← Back to Dashboard</button>
            </motion.div>
          )}

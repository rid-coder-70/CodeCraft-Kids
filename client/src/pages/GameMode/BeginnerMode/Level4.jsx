...
import {
  FaQuestionCircle, FaChevronRight, FaLightbulb, FaGamepad,
  FaCheckCircle, FaStar, FaArrowLeft, FaGem, FaFire,
  FaTrophy, FaGraduationCap, FaBrain, FaTimesCircle,
  FaGift, FaRobot, FaUser, FaMicrophone, FaExclamationTriangle,
  FaCalculator, FaPaperPlane,
} from "react-icons/fa";
...
const LEARN_CARDS = [
  {
    icon: <FaQuestionCircle className="text-amber-500 text-xl" />,
    title: "Basic Input",
    color: "bg-amber-50 border-amber-200",
    code: true,
  },
  {
    icon: <FaCalculator className="text-orange-500 text-xl" />,
    title: "Input + Math (Convert to Number)",
    color: "bg-orange-50 border-orange-200",
    code2: true,
  },
  {
    icon: <FaExclamationTriangle className="text-red-500 text-xl" />,
    title: "Important: input() returns text!",
    color: "bg-red-50 border-red-200",
    note: 'input() always gives you text (str). To do math, wrap it: int(input("Age? "))',
  },
];
...
export default function Level4() {
  ...
  const [chatMessages, setChatMessages] = useState([{ who: "robot", text: "Hi! What is your name?" }]);
  ...
  const handleChatSend = () => {
    ...
    if (chatStage === 0) {
      ...
      setTimeout(() => {
        setChatMessages((prev) => [...prev, { who: "robot", text: `I only know your name and age for now! Python's input() reads what you tell it.` }]);
      }, 600);
    }
    setChatInput("");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fffbeb] via-[#fef9c3] to-[#fef3c7] font-sans overflow-x-hidden">
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-3 flex items-center justify-between shadow-sm">
        <button onClick={() => navigate("/dashboard")} className="flex items-center gap-2 text-gray-400 font-bold text-sm hover:text-amber-500 transition-colors">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            <motion.div key="intro" {...fadeUp} className="text-center">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200, delay: 0.2 }}
                className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-400 to-orange-400 text-white px-8 py-2 rounded-full font-black text-sm mb-6 shadow-lg shadow-amber-200">
                <FaQuestionCircle /> LEVEL 4 — ASK QUESTIONS
              </motion.div>
              ...
              <h1 className="text-4xl font-black text-gray-900 mb-4 flex items-center justify-center gap-3" style={{ fontFamily: FONT }}>
                Make Python Talk to You! <FaMicrophone className="text-amber-500" />
              </h1>
          ...
          {step === 1 && (
            <motion.div key="learn" {...fadeUp}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-black text-gray-900 text-xl" style={{ fontFamily: FONT }}>Learn: input()</h2>
                  <p className="text-gray-400 text-sm font-semibold">Make your program interactive</p>
                </div>
              </div>
                ...
                <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 }}
                  ...
                  {card.note && (
                    <div className="flex items-start gap-2">
                      <FaExclamationTriangle className="text-red-500 text-sm mt-0.5 shrink-0" />
                      <p className="text-sm font-semibold text-red-700 leading-relaxed">{card.note}</p>
                    </div>
                  )}
                </motion.div>
              ))}
            ...
            <motion.div key="chat" {...fadeUp}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-black text-gray-900 text-xl flex items-center gap-2" style={{ fontFamily: FONT }}>
                    Chat with PyBot! <FaRobot className="text-amber-400" />
                  </h2>
                  <p className="text-gray-400 text-sm font-semibold">Experience input() in action</p>
                </div>
              </div>

              {/* Code preview */}
              <div className="bg-[#0f172a] rounded-2xl p-4 font-mono text-xs leading-loose mb-4 border border-amber-900">
                <div className="text-gray-400 font-sans font-bold mb-2 text-[10px] uppercase tracking-wider flex items-center gap-1">
                  <FaRobot className="text-amber-400" /> Python code running:
                </div>
                <div><span className="text-amber-400">name</span> = <span className="text-green-400">input</span>(<span className="text-yellow-300">"What is your name? "</span>)</div>
                <div><span className="text-amber-400">age</span> = <span className="text-blue-300">int</span>(<span className="text-green-400">input</span>(<span className="text-yellow-300">"How old are you? "</span>))</div>
              ...
              <div className="bg-white rounded-2xl border border-amber-100 shadow-sm overflow-hidden mb-4">
                ...
                <div className="border-t border-amber-100 p-3 flex gap-2">
                  ...
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleChatSend}
                    className="px-4 py-2 bg-amber-500 text-white font-black rounded-xl text-sm hover:bg-amber-600 transition-colors flex items-center gap-1">
                    <FaPaperPlane /> Send
                  </motion.button>
                </div>
              </div>
              ...
              {(userName || userAge) && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-amber-50 rounded-2xl p-4 border border-amber-200 mb-4">
                  <div className="text-xs font-black text-amber-700 uppercase tracking-wider mb-2 flex items-center gap-1">
                    <FaRobot className="text-amber-500" /> Python's Memory (Variables):
                  </div>
          ...
          {step === 3 && (
            <motion.div key="quiz" {...fadeUp}>
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 bg-yellow-100 rounded-2xl flex items-center justify-center text-amber-500 text-xl"><FaBrain /></div>
                <div>
                  <h2 className="font-black text-gray-900 text-xl" style={{ fontFamily: FONT }}>Input() Quiz!</h2>
                  <p className="text-gray-400 text-sm font-semibold">Test your knowledge</p>
                </div>
              ...
              {!quizDone ? (
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 text-center">
                  <div className="flex justify-center mb-4">
                    {quizScore === QUIZ.length ? <FaTrophy className="text-yellow-500 text-6xl" /> : <FaStar className="text-yellow-400 text-6xl" />}
                  </div>
                  ...
                  <div className="flex gap-3 justify-center">
                    ...
                    <motion.button whileHover={{ scale: 1.03 }} onClick={() => setStep(4)} className="px-8 py-3 font-black text-white rounded-2xl bg-gradient-to-r from-amber-400 to-orange-400 shadow-lg text-sm flex items-center gap-2">
                      Finish! <FaGift />
                    </motion.button>
                  </div>
                </motion.div>
          ...
          {step === 4 && (
            <motion.div key="complete" {...fadeUp} className="text-center">
              <motion.div animate={{ rotate: [0, -12, 12, 0], scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="text-8xl mb-6 mx-auto w-fit text-amber-500">
                <FaMicrophone />
              </motion.div>
              ...
              <p className="text-gray-500 text-lg mb-6 flex items-center justify-center gap-2">
                Your Python programs can now <strong className="text-amber-600">listen and respond</strong>! <FaRobot className="text-amber-400" />
              </p>
              ...
              <div className="bg-amber-50 rounded-2xl p-5 border border-amber-200 mb-8 text-left">
                <h3 className="font-black text-amber-800 mb-3 flex items-center gap-2"><FaGraduationCap className="text-amber-600" /> What You Learned:</h3>
              ...
              <motion.button whileHover={{ scale: 1.04, y: -3 }} whileTap={{ scale: 0.97 }} onClick={handleComplete} disabled={loading || completed}
                className={`w-full py-4 text-xl font-black text-white rounded-2xl shadow-lg transition-all ${completed ? "bg-amber-400 cursor-default" : "bg-gradient-to-r from-amber-400 to-orange-400 hover:shadow-amber-200 hover:shadow-xl"} disabled:opacity-75 flex items-center justify-center gap-3`}>
                {completed ? <><FaCheckCircle /> Completed! Redirecting...</> : loading ? "Saving..." : <><FaGem /> Claim Your Gems!</>}
              </motion.button>
              <button onClick={() => navigate("/dashboard")} className="mt-4 text-gray-400 font-bold text-sm hover:text-amber-500 transition-colors flex items-center gap-1 mx-auto">
